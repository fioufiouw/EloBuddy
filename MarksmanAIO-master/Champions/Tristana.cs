...
namespace MarksmanAIO.Champions
{
    class Tristana : AIOChampion
    {
        ...
        private static void AntiGapCloser(AIHeroClient sender, Gapcloser.GapcloserEventArgs e)
        {
            ...
            try
            {
                if (!e.Sender.IsValidTarget() || !Value.Use("misc.r.gapcloser") || e.Sender.Type != Player.Instance.Type || !e.Sender.IsEnemy ||
                    Player.Instance.ManaPercent <= Value.Get("misc.r.gapcloser.r.mana"))
                    return;
                _r.Cast(e.Sender);
            }
            catch (Exception a)
