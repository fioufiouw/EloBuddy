...
namespace MarksmanAIO.Champions
{
    class Draven : AIOChampion
    {
        ...
        private static void AutoE()
        {
            ...
            try
            {
                if (Player.Instance.ManaPercent >= Value.Get("misc.spell.e.mana"))
                {
                    if (Value.Use("misc.spell.e.stun") && enemy.IsStunned)
                    {
                        _e.Cast(enemy);
                    }
                    if (Value.Use("misc.spell.e.snare") && enemy.IsRooted)
                    {
                        _e.Cast(enemy);
                    }
                    if (Value.Use("misc.spell.e.charm") && enemy.IsCharmed)
                    {
                        _e.Cast(enemy);
                    }
                    if (Value.Use("misc.spell.e.taunt") && enemy.IsTaunted)
                    {
                        _e.Cast(enemy);
                    }
                    if (Value.Use("misc.spell.e.fear") && enemy.IsFeared)
                    {
                        _e.Cast(enemy);
                    }
                }
                ...
                foreach (
                    Obj_AI_Turret turret in
                        EntityManager.Turrets.Allies.Where(
                            turret => turret.LastTarget().Equals(fuccboi) && fuccboi.Distance(turret) <= 1200))
                {
        ...
        private static void CatchAxe()
        {
            ...
            if (Value.Get("misc.axe.mode") == 1 && Orbwalker.ActiveModesFlags == Orbwalker.ActiveModes.None ||
                Value.Get("misc.axe.mode") == 2 && !Orbwalker.ActiveModesFlags.HasFlag(Orbwalker.ActiveModes.Combo))
                return;
        ...
        public override void LastHit()
        {
            var source =
                EntityManager.MinionsAndMonsters.EnemyMinions.Where(
                    m => m.IsValidTarget(_e.Range) && Player.Instance.GetSpellDamage(m, SpellSlot.E) > m.TotalShieldHealth())
                    .FirstOrDefault();
