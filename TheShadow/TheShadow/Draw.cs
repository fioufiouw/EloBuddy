...
namespace TheShadow
{
    class Draw
    {
        ...
        static public void OnDraw(EventArgs args)
        {
            ...
            {
                if (MainShadow._Q.IsReady() && MainShadow._W.IsReady() && MainShadow._E.IsReady() && MainShadow._R.IsReady())
                {
                    Drawing.DrawText(Drawing.WorldToScreen(Player.Instance.Position) - new Vector2(30, -30), Color.GhostWhite, "Full Combo is Ready", 2);
                }
                else
                {
                    Drawing.DrawText(Drawing.WorldToScreen(Player.Instance.Position) - new Vector2(30, -30), Color.GhostWhite, "Full Combo is NOT Ready", 2);
                }
                if (DamageCalc.dmgCalc() > target.Health)
                {
                    Drawing.DrawText(Drawing.WorldToScreen(Player.Instance.Position) - new Vector2(30,100), Color.GhostWhite, "Target + Combo = RIP", 2);
                }
            }
        }
    }
