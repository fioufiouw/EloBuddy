...
namespace SharpVision
{
    ...
    internal class Vision
    {
        ...
        public Vision()
        {
            ...
            wards.AddRange(rtbj);
            wards.AddRange(btrj);
            menu = MainMenu.AddMenu("SharpVision", "SharpVision");
            //Orbwalker = new Orbwalking.Orbwalker(menu.SubMenu("Orbwalking"));
            menu.Add("PlaceWard", new KeyBind("Ward Closest Position to Cursor", false, KeyBind.BindTypes.HoldActive, 84));
            menu.Add("wVision", new CheckBox("Show Ward Vision?"));
            menu.Add("hVision", new CheckBox("Show Hero/Minion Vision?"));
            ...
            menu.Add("v3add", new KeyBind("Show Predicted Vision", false, KeyBind.BindTypes.HoldActive, 71));
            menu.Add("keypressward", new KeyBind("Auto-Ward on Combo", false, KeyBind.BindTypes.HoldActive, 32));
            menu.Add("toggleward", new KeyBind("Toggle Auto-Ward", true, KeyBind.BindTypes.PressToggle, 84));
            menu.Add("permashow", new CheckBox("Permashow Ward Positions?"));
            menu.Add("autobuywards", new CheckBox("Auto buy wards?"));
            k = menu.AddSubMenu("troll mode: Ok... if you insist...", "lol");
            k.Add("legacymode", new CheckBox("Legacy Mode"));
            
            string[] stealthunits = new[] {"Kha'Zix", "LeBlanc", "Renger", "Shaco", "Talon", "Twitch", "Vayne", "Wukong"};
            foreach (var l in ObjectManager.Get<AIHeroClient>().Where(x => x.IsEnemy))
            {
        ...
        private void Obj_AI_Base_OnCreate(GameObject sender, EventArgs args)
        {
            if (Environment.TickCount - wardtime > 8000)
            {
                if (sender.IsEnemy)
                {
                    ...
                    {
                        if (Item.HasItem(2043))
                        {
                            Item.UseItem(2043, sender.Position);
        ...
        {
            var keypresson = menu["keypressward"].Cast<KeyBind>().CurrentValue;
            var toggled = menu["toggleward"].Cast<KeyBind>().CurrentValue;
            if (Environment.TickCount - wardtime > 8000)
            {
                if (sender.IsEnemy)
                {
                    ...
                    if (spellname == "deceive")
                    {
                        ...
                        if (keypresson || toggled)
                        {
                                ...
                                650)
                            {
                                if (Item.HasItem(2043))
                                {
                                    Item.UseItem(2043, args.Start + (args.End - args.Start).Normalized()*400);
                                    wardtime = Environment.TickCount;
                                }
                                else if (Item.HasItem(3364))
                                {
                                    Item.UseItem(3364, args.Start + (args.End - args.Start).Normalized()*400);
                                    wardtime = Environment.TickCount;
                                }
                            }
                        }
                        ...
                        if (sender.HasBuff("VayneInquisition"))
                        {
                                ...
                                650)
                            {
                                if (keypresson || toggled)
                                {
                                    if (Item.HasItem(2043))
                                    {
                                        Item.UseItem(2043, args.Start + (args.End - args.Start).Normalized()*300);
                                        wardtime = Environment.TickCount;
                                    }
                                    else if (Item.HasItem(3364))
                                    {
                                        Item.UseItem(3364, args.Start + (args.End - args.Start).Normalized()*300);
                                        wardtime = Environment.TickCount;
                                    }
                                }
                            }
                    ...
                    else if (spellname == "summonerflash" || spellname == "khazixe" || spellname == "ezreale")
                    {
                        ...
                        if (ObjectManager.Player.Distance(args.End) < 1100)
                        {
                            if (keypresson || toggled)
                            {
                                var wardslot = CommonUtils.GetWardSlot();
                                if (wardslot != null)
                                {
                                    Item.UseItem((int) wardslot.Id,
                                        args.Start + (args.End - args.Start).Normalized()*425);
                                    wardtime = Environment.TickCount;
                                }
                                else if (Item.HasItem(2043))
                                {
                                    Item.UseItem(2043, args.Start + (args.End - args.Start).Normalized()*425);
                                    wardtime = Environment.TickCount;
                                }
                                else if (Item.HasItem(3364))
                                {
                                    Item.UseItem(3364, args.Start + (args.End - args.Start).Normalized()*425);
                                    wardtime = Environment.TickCount;
                                }
                             ...
                             spellname == "akalismokebomb")
                    {
                        ...
                        startpos.Add(time, sender.Position); if (keypresson || toggled)
                        {
                            if (ObjectManager.Player.Distance(sender.Position) < 650)
                            {
                                ...
                                {
                                    Item.UseItem(3364, sender.Position);
                                    wardtime = Environment.TickCount;
                                }
                            }
                        }
        ...
        private static Item ScryingOrb2 = new Item(3363, 3500f);
        private static Item ScryingOrb = new Item(3342, 3500f);
        ...
        private void Game_OnDraw(EventArgs args)
        {
            ...
            if (Player.IsInShopRange() && menu["autobuywards"].Cast<CheckBox>().CurrentValue && Game.Time > 200)
            {
                if (Player.Gold < 135)
                {
            ...
            foreach (var s in blinkpos)
            {
                if (Environment.TickCount - s.Key < 3000)
                {
                    var end = Drawing.WorldToScreen(s.Value);
                    var start = Drawing.WorldToScreen(startpos[s.Key]);
                    Drawing.DrawLine(start, end, 1, System.Drawing.Color.Yellow);
                    Drawing.DrawCircle(s.Value, 125, System.Drawing.Color.Red);
                }
                else
                {
                    blinkpos.Remove(s.Key);
                    startpos.Remove(s.Key);
                }
            }
                ...
                if (enemy.Distance(Player.ServerPosition) <= 1200) //check real ward range later
                {
                    ...
                    if (bestWardPos != enemy.ServerPosition && bestWardPos != Vector3.Zero)
                    {
                        if (!enemy.IsVisible && (lastTimeWarded == 0 || Environment.TickCount - lastTimeWarded > 1000) &&
                            ...
                            menu["keypressward"].Cast<KeyBind>().CurrentValue)
                        {
                            var wardslot = CommonUtils.GetWardSlot();
                            if (wardslot != null)
                            {
                                Item.UseItem((int) wardslot.Id, bestWardPos);
                            }
                        ...
                        new Circle() { BorderWidth = 1, Color = System.Drawing.Color.Red, Radius = radius}.Draw(bestWardPos);
                    }
            ...
            if (menu["permashow"].Cast<CheckBox>().CurrentValue)
            {
                foreach (var s in wards)
                {
                    Drawing.DrawCircle(s, 125, System.Drawing.Color.Red);
                }
            }

            if (menu["PlaceWard"].Cast<KeyBind>().CurrentValue)
            {
                if (!menu["permashow"].Cast<KeyBind>().CurrentValue)
                {
                    foreach (var s in wards)
                    {
                        Drawing.DrawCircle(s, 100, System.Drawing.Color.Red);
                    }
                }
                var wardslot = CommonUtils.GetWardSlot();
                    ...
                    wardslot.Id != (ItemId)3350)
                {
                    if (Environment.TickCount - lastwardtime > 800)
                    {
                        ...
                        foreach (var s in wards)
                        {
                            var dist = s.Distance(Game.CursorPos);
                            if (dist < closestpos)
                            {
                                closestpos = dist;
                                wardpos = s;
                        ...
                        Item.UseItem((int)wardslot.Id, wardpos);
                        lastwardtime = Environment.TickCount;
                    }
                }
            }
            
            if (k["legacymode"].Cast<CheckBox>().CurrentValue)
            {
                var wardslot = CommonUtils.GetWardSlot();
                if (wardslot != null && wardslot.Id != (ItemId) 2050 && wardslot.Id != (ItemId) 3154 &&
                    wardslot.Id != (ItemId) 3350)
                {
                    if (Environment.TickCount - lastwardtime > 800)
                    {
                        foreach (var s in wards)
                        {
                            if (s.Distance(Player.Position) < 650)
                            {
                                Item.UseItem((int) wardslot.Id, s);
                                lastwardtime = Environment.TickCount;
                            }
                        }
                    }
            ...
            if (menu["v3add"].Cast<KeyBind>().CurrentValue)
            {
                //if (menu.Item("mVision").GetValue<bool>())
                //{
                //    showVision(System.Drawing.Color.Blue, 1100);
                //}
                if (menu["wVision"].Cast<CheckBox>().CurrentValue)
                {
                    showVision(System.Drawing.Color.LightGreen, 900);
                }
                if (menu["hVision"].Cast<CheckBox>().CurrentValue)
                {
